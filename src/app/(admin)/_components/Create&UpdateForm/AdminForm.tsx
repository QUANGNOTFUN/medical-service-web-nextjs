...
export default function AdminForm<T>({ initialData, title, fields, onClose, onSubmit, submitLabel }: AdminFormProps<T>) {
	...
	return (
        ...
        outline outline-black/10 rounded-lg shadow-lg z-50
        transition-all duration-300
      `}
		>
			<div className={"form-header bg-violet-300 dark:bg-gray-900 p-4 rounded-t-lg"}>
				...
				<button
					className={"absolute top-0 right-0 p-0.5 rounded-lg bg-violet-300 dark:bg-gray-900 hover:bg-neutral-100 dark:hover:bg-gray-700"}
					onClick={onClose}
				>
					<SquareX className={"w-6 h-6 text-stone-100 dark:text-gray-200 hover:text-violet-800 dark:hover:text-gray-100"}/>
				</button>
			</div>

			<form onSubmit={handleSubmit} className={"space-y-4 bg-white dark:bg-gray-700 p-2 rounded-b-lg"}>
				{fields.map((field) => (
